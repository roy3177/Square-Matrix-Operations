...
using namespace std;
using namespace mat; //Using SquareMat

//Return the size of the metrix:
...
SquareMat::SquareMat(int n){
    ...
    //Check if the size of the matrix is legall:
    if(n<=0){
        throw invalid_argument("Our matrix size must be positive!");
    }
    
    this->size=n;
    this->data=new double[n*n]; //Allocate array of n^2 elements.

    for(int i=0;i<n*n;i++){
        this->data[i]=0; //Initialiazing all the elements to 0.
    }
}
...
SquareMat& SquareMat::operator=(const SquareMat& other){
    //If assigning to self-->return it:
    if(this==&other){
        return *this;
    }
...
SquareMat& SquareMat::operator=(SquareMat&& other)noexcept{
    //Checking for self-assignment in move:
    if(this==&other){
        return *this;
    }
    ...
    size=other.size;
    data=other.data;
    other.data=nullptr;//Null out other--->for not free memory that move already.
    other.size=0;
    return *this;

}

//This is a  write access version of the operator []:
double& SquareMat::operator[](int index){
    //Check if the index is valid:
    if(index<0 || index>=size*size){
        throw out_of_range("Index out of bounds in SquareMat::operator[]");
    }
    
    //Return a reference to the element with our index.
    //Enables both reading and writing to this element with our index
    return data[index];
}

//This is a read access version of the operator [] :
const double& SquareMat::operator[](int index) const{
    ...
    //Return a reference to the element with our index.
    //Enables only reading(const):
    return data[index];
}

//Destructor (~):
mat::SquareMat::~SquareMat(){
    delete[] data;
}

//This is a method for operator+ (mat1+mat2):
mat::SquareMat mat::SquareMat:: operator+(const SquareMat& other)const{
...
mat::SquareMat mat::SquareMat::operator-()const{

    //We gonna create new matrix at the same size:
    ...
    return ans; //Now we return unary matrix
}

//Multiply our matrix with scalar-->matrix*scalar:
mat::SquareMat mat::SquareMat::operator*(double scalar)const{
    //We gonna create new matrix at the same size:
    SquareMat ans(this->size);
...
namespace mat{
//Multiply our matrix with scalar-->scalar*natrix:
mat::SquareMat  operator*(double scalar,const mat::SquareMat& mat){
    return mat*scalar;
}
}
...
mat::SquareMat mat::SquareMat::operator%(int scalar )const{
...
mat::SquareMat mat::SquareMat::operator^(int power)const{
...
mat::SquareMat mat::SquareMat::operator~()const{
    ...
    SquareMat trans(this->size); //Thus is our transport matrix.

    for(int i=0;i<size;i++){
        for(int j=0;j<size;j++){
            trans[j*size+i]=this->data[i*size+j]; //trnas[i][j]=mat[j][i]
        }
    }
...
double SquareMat::helperDet(const double* mat,int n)const{
    ...
    double det=0; //variable for the determinant result.
...
//Operator << --->print our matrix:
    std::ostream& operator<<(std::ostream& os,const mat:: SquareMat& mat){
